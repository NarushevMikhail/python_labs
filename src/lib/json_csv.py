...
def json_to_csv(json_path: str, csv_path: str) -> None:
    ...
    ):  # проверяет что все элементы должны быть списком
    ...
    if data:  # проврека данных
        ...
        remaining_keys = sorted(
            all_keys - set(first_item_keys)
        )  # находит ключи которых нет в 1 элементе
    ...
    try:  # Запись в CSV
        with csv_file.open("w", newline="", encoding="utf-8") as f:
            ...
            )  # класс для записи CSV из словарей, список названий столбцов в правильном порядке
            writer.writeheader()  # Записывает первую строку CSV с названиями столбцов
            for row in data:
                complete_row = {
                    key: row.get(key, "") for key in fieldnames
                }  # значение по ключу из текущей строки
