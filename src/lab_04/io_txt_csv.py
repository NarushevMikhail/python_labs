...
def write_csv(
    ...
    ) as f:  # w - открыть файл для записи, mewline - настройка для работы с csv файлами
        w = csv.writer(f)
...
def ensure_parent_dir(path: str | Path) -> None:  # принимает путь, ничего не возвращает
    ...
    os.makedirs(
        parent_directory, exist_ok=True
...
def test():
    ...
    write_csv(
        rows=[],
        path=r"C:\Users\narus\OneDrive\Рабочий стол\лабароторные работы\Программирование\репозиторий\python_labs\python_labs-1\src\data\check_empty.csv",
        header=None,
    )
