...
<strong>Normalize</strong><div> Данная функция, для начала она приводит текст к нижнему регистру, затем заменяет все буквы 'ё' на 'е', только потом заменяет все управляющие символы (табуляции, переносы строк) на проблеы. После этого удаляем все лишние пробелы через split, оставляя только одинарные между словами.</div>
<strong>tokensize</strong><div>Используеn регулярные выражения, благодаря которым будет находится слова с дефисом. Возвращает список токенов.</div>
    ## `count_freq`
